...
from resnet_cifar import ResNet50, ResNet34, ResNet18, WideResNet
...
def get_images(net, bs=256, epochs=1000, idx=-1, var_scale=0.00005,
    ...
    if random_labels:
        ...
        targets = torch.LongTensor([0]*(bs//10) + [1]*(bs//10) + [2]*(bs//10) + [3]*(bs//10) + 
                                   [4]*(bs//10) + [5]*(bs//10) +
                                   [6]*(bs//10) + [7]*(bs//10) + [8]*(bs//10) + [9]*(bs//10) + 
                                    list(range((bs%10)))).to('cuda')
    ...
    for epoch in range(epochs+1):
        ...
        if debug_output and epoch % 400 == 0:
            print(f"It {epoch}\t Losses: total: {loss.item():3.3f},\ttarget: {loss_target:3.3f} \tR_feature_loss unscaled:\t {loss_distr.item():3.3f}")
            for i, dd in enumerate(inputs.data):
                p = './{}/epoch-{}/{}/'.format(prefix, epoch, label2name[int(targets[i])])
                if not os.path.exists(p):
                    os.makedirs(p)
                l = len([name for name in os.listdir(p) if os.path.isfile(os.path.join(p, name))])
                vutils.save_image(dd.clone(), p + '{}.png'.format(l), normalize=True, scale_each=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--teacher_weights', default="'./ckpt.pth'", type=str, help='path to load weights of the model')

    args = parser.parse_args()


    net_teacher = WideResNet(depth=34, num_classes=10, widen_factor=10, dropRate=0.0)
    net_student = ResNet18()

    print("WideResNet: " + str(count_parameters(net_teacher)))
    print("ResNet18: " + str(count_parameters(net_student)))
